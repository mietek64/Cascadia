#!/usr/bin/env python3
"""
cascadia  —  Matrix waterfall terminal animation

  cascadia          show this help screen
  cascadia -s       start animation
  cascadia -c       config editor
  cascadia -p       preset browser
  cascadia -s -f x.json   start with custom config
  Q / Ctrl-C  →  quit
"""
...
if sys.platform == "win32":
    _CFG_HOME = os.path.join(os.environ.get("APPDATA", os.path.expanduser("~")), "cascadia")
else:
    _CFG_HOME = os.path.expanduser("~/.config/cascadia")
...
def build_styles(cfg: dict, cycled_color: str = "") -> dict:
    ...
    raw_tbg = cfg.get("title_bg", "")
    # Only apply a background colour if the user explicitly set one
    title_style = (
        Style(color=tc, bgcolor=safe_color(raw_tbg, "black"), bold=True)
        if raw_tbg and is_valid_color(raw_tbg)
        else Style(color=tc, bold=True)
    )
    return {
        ...
        with Live("", console=console, screen=True,
            ...
            while not stop_event.is_set():
                ...
                if (cols, rows) != last_size:
                    streams.clear()
                    styles    = build_styles(cfg)
                    tilt_pad  = int(rows * abs(tilt) * 0.25) if tilt != 0 else 0
                    last_size = (cols, rows)
                ...
                # Expand spawn range off-screen to fill gaps caused by tilt drift
                for c in range(-tilt_pad, cols + tilt_pad):
                    col_density = density
                    if wave:
...
def show_splash() -> None:
    """Displayed when cascadia is run with no arguments."""
    console = Console()
    G  = "#00cc44"
    DG = "#005522"
    W  = "bright_white"
    ...
    if HAS_FIGLET:
        try:
            art_lines = pyfiglet.Figlet(font="doom").renderText("CASCADIA").rstrip("\n").split("\n")
        except Exception:
            art_lines = ["  C A S C A D I A"]
    else:
        art_lines = ["  M A S C A D I A  —  terminal waterfall"]
    ...
    commands = [
        ("cascadia -s",          "start the animation"),
        ("cascadia -c",          "open config editor"),
        ("cascadia -p",          "browse & run presets"),
        ("cascadia -s -f x.json","start with custom config file"),
        ("",                   ""),
    ...
    # ── Tips ──────────────────────────────────────────────────────────────────
    tips = [
        "Run  cascadia -c  to change color, speed, charset and more.",
        "Try presets first — run  cascadia -p  to pick one.",
        ...
        "Use  cascadia -s -f mytheme.json  to run a separate theme file.",
    ]
...
def _draw_config_editor(console: Console, cfg: dict, path: str, modified: bool) -> None:
    ...
    hdr.append("CASCADIA CONFIG", style=Style(color=sc, bold=True))
    hdr.append("  ░▒▓",         style=Style(color=sc, dim=True))
...
def _draw_preset_browser(console: Console, presets: list[dict], sc: str) -> None:
    ...
    hdr = Text(justify="center")
    hdr.append("▓▒░  ",          style=Style(color=sc, dim=True))
    hdr.append("CASCADIA PRESETS", style=Style(color=sc, bold=True))
    hdr.append("  ░▒▓",          style=Style(color=sc, dim=True))
...
# ═════════════════════════════════════════════════════════════════════════════
#  Self-installer  (cascadia --install)
# ═════════════════════════════════════════════════════════════════════════════
...
def run_install() -> None:
    ...
    console.print()
    console.print(f"  [bold {G}]░▒▓  CASCADIA INSTALLER  ▓▒░[/]")
    console.print()
    ...
    src_path    = sys.executable if getattr(sys, "frozen", False) else os.path.abspath(__file__)
    install_dir = r"C:\tools"
    dest        = os.path.join(install_dir, "cascadia.exe")
    ...
    try:
        ...
        console.print(f"  [bold green][+][/] Copied cascadia.exe → {dest}")
    except Exception as e:
    ...
    console.print(f"    [bold {G}]cascadia[/]      [dim]show help[/]")
    console.print(f"    [bold {G}]cascadia -s[/]   [dim]start animation[/]")
    console.print(f"    [bold {G}]cascadia -c[/]   [dim]config editor[/]")
    console.print(f"    [bold {G}]cascadia -p[/]   [dim]preset browser[/]")
    console.print()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description="Cascadia waterfall terminal animation",
    )
